  ...
  return (
    <div className="device-detail">
      ...
      {asset.command_history && asset.command_history.length > 0 && (
        <div className="command-history-box">
            ...
            {asset.command_history.slice(0, 2).map(cmd => {
              ...
              return (
                <div key={cmd.id} className="command-history-item">
                  <span className="command-history-cmd">{commandName}</span>
                  <div style={{ display: 'flex', alignItems: 'center' }}>
                    <span className="command-history-status" style={{ 
      ...
      <main className={`content ${activeTab === "tracker" ? "content-full" : ""} ${sidebarCollapsed ? "main-expanded" : ""}`}>
        ...
        {error && !permBlocked && (
          <div className="error-banner" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <strong>⚠️ VoltCred API Error:</strong> {error}
              <div style={{ fontSize: 12, marginTop: 4, opacity: 0.8 }}>
                {assets.length > 0 
                  ? 'Showing last successful data. Auto-refreshing every 30 seconds...' 
                  : 'Unable to fetch vehicle data. This may be due to VoltCred API returning invalid data (observed: null).'}
              </div>
            </div>
            <button 
              className="refresh-btn" 
